...
def main(model, test, args):
    ...
    z_ptypes = model.concept_dists # Shape (N_p, d_z)
    ...
    for yi in y.unique():
        ...
        for i in range(z_ptypes.shape[0]):
            for j in range(Ne):
                sim_mat = cosine_sim_matrix(ze_sub[:,:,j], z_ptypes)

                mean_dist = sim_mat.mean()
                stderr_dist = sim_mat.std() / np.sqrt(sim_mat.flatten().shape[0])

                print('\tP = {}, E = {} \t sim = {:.4f} +- {:.4f}'.format(i, j, mean_dist.item(), stderr_dist.item()))
        print(' ---- ')
