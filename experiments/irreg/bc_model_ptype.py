...
def compute_masked_mean_std(X, times):

    Xc = X.clone()
    tc = times.clone()

    mask = (tc < 1e-5)
    mask_expand = mask.unsqueeze(-1).repeat(1, 1, Xc.shape[-1])

    Xc = torch.where(mask_expand, float('nan'), Xc)
    mu = Xc.nanmean(dim = 1)
    Xc_zeros = torch.where(mask_expand, 0, Xc)
    std = Xc_zeros.std(unbiased = True, dim = 1)
...
def main(args):
    ...
    arch = 'transformer'
    tencoder_path = "/n/data1/hms/dbmi/zitnik/lab/users/owq978/TimeSeriesCBM/experiments/irreg/models/transformer_split={}.pt"
    ...
    for i in range(1, 6):
        # if (i == 3):
        #     continue
        D = process_Synth(split_no = i, device = device, base_path = '/n/data1/hms/dbmi/zitnik/lab/users/owq978/TimeSeriesCBM/datasets/SeqCombMVIrreg')
        ...
        mu, std = compute_masked_mean_std(D['train_loader'].X, D['train_loader'].times)
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr = 3e-3, weight_decay = 0.001) #For regular
