...
'''
Experiment 5 - test mlp aggregator
'''
n_aug = 100
gauss_std = 0.5
rand_mask = 0.9
mshift = True
tshift = (0.05, 50)
print('Running variation std={} rm={} ms={} ts={}'.format(gauss_std, rand_mask, mshift, tshift))
...
for i in range(1, 6):
    ...
    abl_params = AblationParametersConcepts(
            equal_g_gt = False, 
            g_pret_equals_g = False, 
            label_based_on_mask = True,
            ze_closeness_to_concepts = True, 
            concept_matching = True,
            mlp_aggregation = True,
            use_concept_decoder = True
        )
    ...
    spath = 'models/exp5_std={}_rm={}_ms={}_wdec.pt'.format(gauss_std, rand_mask, mshift)
