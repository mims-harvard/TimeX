...
'''
Experiment 5 - Separate g vs. g_t, but use consistency loss when not equal
    - 2 variations: 
        1. g_t random, separate
        3. g_t init w g, trained separate
'''
variation = 4
assert variation in [1,3,4], 'Due to consistency in experiment notation, do not allow variation outside of 1, 3, and 4'
...
for i in range(1, 6):
    ...
    if (variation == 3) or (variation == 4): # Init w main encoder weights
    ...
    spath = 'models/v6_exp5_var={}_consistency_split={}.pt'.format(variation,i)
