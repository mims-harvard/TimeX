...
def main():
    ...
    model = Modelv3SepExtractor(**config)
    model.to(device)

    model.load_state_dict(sdict)

    f1, _ = eval_mv3(test_tuple, model)
    print('Test F1: {:.4f}'.format(f1))

    visualize(model, test_tuple, show = False, class_num = 0)
    fig = plt.gcf()
    fig.set_size_inches(18.5, 10.5)
    plt.savefig('mv3_side_class1.png', dpi=200)
    plt.show()
