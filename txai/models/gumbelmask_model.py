...
from .mTAN.src.models import enc_mtan_classif
from .masking import KumaGate
...
class STENegInf(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        mfill = mfill.sum(-1)
        #print('nans', torch.isnan(mfill).sum())
        return mfill

    @staticmethod
    def backward(ctx, grad_output):
        #print('nans (backward)', grad_output.isnan().sum())
        return torch.nn.functional.hardtanh(grad_output.unsqueeze(-1).expand(-1, -1, 4))

class GumbelGate(nn.Module):
    def __init__(self, in_features, hidden_dims = 32, out_features = 1):
    ...
    def __init__(self, importance_in_evals = False, get_embeddings = False, 
        ...
        if predictor is not None:
            ...
            encoder_layers = TransformerEncoderLayerInterpret(
                ...
                batch_first = False)
            self.encoder2 = TransformerEncoderInterpret(encoder_layers, self.nlayers)
