...
def generate_seq(T = 500, D = 15, class_num = 0):
    ...
    if class_num == 0: # Null class - make no modifications
        mask_out_flat = (np.random.uniform(low=0.0, high=1.0, size = (T,)) > 0.85)
        mask_out = np.stack([mask_out_flat.copy() for _ in range(D)], axis = -1)
        samp = np.where(mask_out, -1e8, samp)
        return samp, [None], mask_out_flat
    ...
    # Randomly mask 15% of the sensors:
    mask_out_flat = (np.random.uniform(low=0.0, high=1.0, size = (T,)) > 0.85)
    mask_out = np.stack([mask_out_flat.copy() for _ in range(D)], axis = -1)
    samp = np.where(mask_out, -1e8, samp)

    # Iterate through coords, remove ones that were
    coords_new = []
    for xi, yi in coords:
        if samp[xi,yi] > -1e7:
            coords_new.append((xi, yi))

    return samp, coords_new, mask_out_flat # Shape (T,)
...
def generate_spike_dataset(N = 1000, T = 500, D = 15):
    ...
    for i, n in enumerate(class_count):
        for _ in range(n):
            ...
            Xi, locs, mask_out_flat = generate_seq(T, D, class_num = i)
            X[total_count,:,:] = Xi
            times[total_count,:] = np.where(mask_out_flat, -1e6, np.arange(1,T+1)) # Steadily increasing times, mask out irreg
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        torch.save(dataset, base + '/SeqCombMVIrreg/split={}.pt'.format(i + 1))
