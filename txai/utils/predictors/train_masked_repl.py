...
def train_masked_repl(
        ...
        freeze_predictor = False): # TODO: implement
    '''
    ...
    for epoch in range(num_epochs):
        ...
        for X, times, y in train_loader:
            ...
            out, mask, to_opt, attn_weights = model(X.transpose(0, 1), times.transpose(0,1), captum_input = False)
        ...
        with torch.no_grad():
            ...
            if validate_by_step:
                ...
                for i in range(X.shape[1]):
                    pred[i,:], mask[i,:], _, _  = model(X[:,i,:], times[:,i].unsqueeze(-1))
                    #pred[i,:], sat_mask[i,:], _ = model(X[:,i,:], times[:,i].unsqueeze(-1))
            else:
                pred, mask, _, _ = model(X, times, captum_input = False)
