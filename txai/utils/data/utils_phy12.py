...
def tensorize_normalize(P, y, mf, stdf, ms, ss):
    ...
    return P_tensor.float(), P_static_tensor.float(), P_time.float(), y_tensor
...
def evaluate_standard(model, P_tensor, P_time_tensor, P_static_tensor, batch_size=100, n_classes=2, static=1, examples_first_time = False):
    P_tensor = P_tensor.cuda()
    P_time_tensor = P_time_tensor.cuda()
    # if examples_first_time: 
    #     P_time_tensor = P_time_tensor.transpose(0, 1)
    if static is None:
        P_static_tensor = None
    else:
        P_static_tensor = P_static_tensor.cuda()

    # Flip time to calculate lengths if examples come first (needed for Captum)
    lengths = torch.sum((P_time_tensor.transpose(0, 1) if examples_first_time else P_time_tensor) > 0, dim=0)
    out = model.forward(src=P_tensor, static=P_static_tensor, times=P_time_tensor, lengths=lengths)
    return out
