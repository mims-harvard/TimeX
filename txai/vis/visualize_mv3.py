...
def visualize(model, test_tup, n = 3, per_class = False, class_num = None, show = True, sim = False):
    ...
    with torch.no_grad():
        if sim:
            pred, pred_tilde, mask_in, ste_mask, smoother_stats, smooth_src, _ = model(sampX, samp_times, captum_input = False)
        else:
            pred, pred_tilde, mask_in, ste_mask, smoother_stats, smooth_src = model(sampX, samp_times, captum_input = False)
    pred = pred.softmax(dim=1).argmax(dim=1)
    pred_tilde = pred_tilde.softmax(dim=1).argmax(dim=1)
    ...
    title_format1 = 'y={:1d}, yhat={:1d}, yhat_tilde={:1d}'
    ...
    for i in range(n): # Iterate over samples
        ...
        yi = samp_y[i].item()
        pi = pred[i].item()
        pit = pred_tilde[i].item()
        ax[0,i].set_title(title_format1.format(yi, pi, pit))
