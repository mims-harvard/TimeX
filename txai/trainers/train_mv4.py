...
def train_mv4(
    ...
    sim_criterion = torch.nn.TripletMarginLoss(margin = 1.0)
    ...
    for epoch in range(num_epochs):
        ...
        for X, times, y in train_loader:
            ...
            anchor, positive, negative = out_dict['all_z']
            sim_loss = beta_sim * sim_criterion(
                anchor = anchor,
                positive = positive,
                negative = negative,
            )
        ...
        print(f'Epoch: {epoch}: Sparsity = {sparse:.4f} \t Exp Loss = {exp} \t Clf Loss = {clf:.4f} \t Logit Loss = {logit:.4f}')
        #exit()
