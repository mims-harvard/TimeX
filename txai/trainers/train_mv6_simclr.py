...
def train_mv6(
    ...
    for epoch in range(num_epochs):
        ...
        for X, times, y, ids in train_loader: # Need negative sampling here
            ...
            # Call bilinear scorer:
            # pos_scores = model.score_contrastive(org_embeddings, conc_embeddings)
            # con_emb_expand = conc_embeddings.unsqueeze(1).repeat(1, neg_embeddings.shape[1], 1)
            # neg_scores = model.score_contrastive(neg_embeddings, con_emb_expand)

            sim_loss = sim_criterion(conc_embeddings, org_embeddings, neg_embeddings)
            ...
            exp_loss = beta_exp * (loss_dict['mask_loss'] + loss_dict['ent_loss'] + loss_dict['p_loss'])
            eloss_list = [loss_dict['mask_loss'].detach().clone().item(), loss_dict['ent_loss'].detach().clone().item()]
